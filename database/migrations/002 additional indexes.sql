...
ALTER TABLE tickets
  ADD INDEX IF NOT EXISTS idx_tickets_bodega_estado (bodega_id, estado),
  ADD INDEX IF NOT EXISTS idx_tickets_bodega_fecha  (bodega_id, fecha_creacion);

-- Índice compuesto para búsqueda de eventos por deporte + fecha (listado principal)
ALTER TABLE eventos
  ADD INDEX IF NOT EXISTS idx_eventos_deporte_fecha (deporte, fecha_inicio, estado);

-- Índice para notificaciones no leídas por rol (panel admin)
ALTER TABLE notificaciones
  ADD INDEX IF NOT EXISTS idx_notificaciones_rol_leido (destinatario_rol, leido, created_at);

-- Índice para auditoría por entidad afectada
ALTER TABLE auditoria_logs
  ADD INDEX IF NOT EXISTS idx_auditoria_entidad (entidad_afectada, entidad_id);

-- Índice para cierre de caja por bodega y fecha
ALTER TABLE cierre_caja
  ADD INDEX IF NOT EXISTS idx_cierre_caja_bodega_fecha (bodega_id, fecha);
